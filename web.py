...
def isDownlaodable(youtube_link):
    try:
        with yt_dlp.YoutubeDL({'format': 'bestaudio', "quiet":True, "noplaylist":True, "source_address": "152.58.79.181"}) as ydl:
            dur = None
...
def download_youtube_audio(youtube_link):
    ...
    if isDownlaodable(youtube_link):
        try:
            with yt_dlp.YoutubeDL({'format': 'bestaudio/best', 'outtmpl': 'uploaded_files/' + uu + '.%(ext)s', "quiet":True, "noplaylist":True, "source_address": "152.58.79.181"}) as ydl:
...
def main():
    ...
    st.markdown("# Bookmark new [Website](https://lofi-convertor.samevibe.in/)")
